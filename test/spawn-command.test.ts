...
test("spawn creates isolated branch, canonical record, runs pi, and resumes its worktree", async (context) => {
	...
	const launched = limenWithSession(scratch, "coordinator-a", "spawn", "--detached", "--label", "F001 implementation", "make commit");
	...
	const resumed = limen(scratch, "spawn", "--detached", "continue work", "--branch", `limen/${id}`);
...
test("failure is durable and detailed jobs render facts", async (context) => {
	...
	context.after(scratch.cleanup);
	limen(scratch, "init");
	const id = onlyJobId(limen(scratch, "spawn", "--detached", "fail now").stdout);
...
test("a project worker overlay replaces the package birth text", async (context) => {
	...
	const id = onlyJobId(limen(scratch, "spawn", "--detached", "no model default").stdout);
	await waitForState(scratch.root, id, "done");
...
test("review gets fresh detached worktree and reviewer birth text", async (context) => {
	...
	const review = onlyJobId(limen(scratch, "spawn", "--detached", "--review", "--branch", branch, "inspect candidate").stdout);
...
test("stage model defaults respect review roles and explicit overrides", async (context) => {
	...
	const packageDefault = onlyJobId(limen(scratch, "spawn", "--detached", "package model default").stdout);
	...
	const reviewDefault = onlyJobId(limen(scratch, "spawn", "--detached", "--review", "--branch", `limen/${packageDefault}`, "requested review default").stdout);
	...
	const blank = onlyJobId(limen(scratch, "spawn", "--detached", "blank stage default").stdout);
	...
	const worker = onlyJobId(limen(scratch, "spawn", "--detached", "worker model default").stdout);
	await waitForState(scratch.root, worker, "done");
	assert.equal(await modelForJob(scratch.root, worker), "worker-default");
	const review = onlyJobId(limen(scratch, "spawn", "--detached", "--review", "--branch", `limen/${worker}`, "reviewer model default").stdout);
	await waitForState(scratch.root, review, "done");
	assert.equal(await modelForJob(scratch.root, review), "reviewer-default");
	const explicit = onlyJobId(limen(scratch, "spawn", "--detached", "--model", "ticket-specific", "explicit model").stdout);
	await waitForState(scratch.root, explicit, "done");
	assert.equal(await modelForJob(scratch.root, explicit), "ticket-specific");
	const explicitReview = onlyJobId(limen(scratch, "spawn", "--detached", "--review", "--branch", `limen/${worker}`, "--model", "review-specific", "explicit review model").stdout);
...
test("independent jobs can run concurrently and are merely announced", async (context) => {
	...
	context.after(scratch.cleanup);
	limen(scratch, "init");
	const first = onlyJobId(limen(scratch, "spawn", "--detached", "first").stdout);
	const secondLaunch = limen(scratch, "spawn", "--detached", "second");
...
test("prune drops a finished worktree and spawn keeps a resumed one", async (context) => {
	...
	context.after(scratch.cleanup);
	limen(scratch, "init");
	const first = onlyJobId(limen(scratch, "spawn", "--detached", "--label", "keep later", "make commit").stdout);
	...
	assert.ok(worktree);
	await writeFile(join(worktree, "uncommitted.txt"), "keep me\n");
	const second = onlyJobId(limen(scratch, "spawn", "--detached", "make commit").stdout);
	await waitForState(scratch.root, second, "done");
	assert.doesNotMatch(git(scratch.root, "worktree", "list", "--porcelain"), new RegExp(first));
	const resumed = limen(scratch, "spawn", "--detached", "continue work", "--branch", `limen/${first}`);
	assert.equal(resumed.status, 0, resumed.stderr);
	const resumedId = onlyJobId(resumed.stdout);
	await waitForState(scratch.root, resumedId, "done");
...
test("spawn prints failed when the wrapper dies before writing pid", async (context) => {
	...
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const launched = limenWithEnv(scratch, { LIMEN_PI: "" }, "spawn", "--detached", "--label", "boom", "do work");
	assert.equal(launched.status, 0, launched.stderr);
...
test("spawn without pi on PATH fails before worktree add", async (context) => {
	...
	const launched = limenWithEnv(scratch, { PATH: "/nonexistent" }, "spawn", "--detached", "do work");
...
`);
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const launched = limenWithEnv(scratch, { LIMEN_PREFLIGHT: "auth" }, "spawn", "--detached", "--model", "ticket-specific", "do work");
	assert.equal(launched.status, 1);
...
test("LIMEN_PREFLIGHT=auth proceeds when check passes", async (context) => {
	...
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const launched = limenWithEnv(scratch, { LIMEN_PREFLIGHT: "auth" }, "spawn", "--detached", "--model", "ticket-specific", "no model default");
	assert.equal(launched.status, 0, launched.stderr);
	const id = onlyJobId(launched.stdout);
	await waitForState(scratch.root, id, "done");
...
test("task-file and stdin write task.md bytes untouched", async (context) => {
	...
	const fromFile = limen(scratch, "spawn", "--detached", "--task-file", "hand.md", "--label", "file task");
	...
	const fromStdin = limenWithInput(scratch, bytes.toString("utf8"), "spawn", "--detached", "--task-file", "-", "--label", "stdin task");
...
`);
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const numberOnly = limen(scratch, "spawn", "--detached", "--label", "F068", "do work");
	...
	const first = limen(scratch, "spawn", "--detached", "--label", label, "long work");
	...
	const duplicate = limen(scratch, "spawn", "--detached", "--label", label, "long work");
...
test("positional empty backticks or doubled spaces warn once", async (context) => {
	...
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const launched = limen(scratch, "spawn", "--detached", "--label", "warn", "fix `` and  gaps");
	assert.equal(launched.status, 0, launched.stderr);
...
test("git missing from PATH falls back or leaves no job dir", async (context) => {
	...
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const launched = limenWithEnv(scratch, { PATH: `${scratch.fakeBin}:${dirname(process.execPath)}` }, "spawn", "--detached", "--label", "no path git", "do work");
...
test("spawn --role quality loads the packaged preamble", async (context) => {
	...
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const launched = limen(scratch, "spawn", "--detached", "--role", "quality", "--label", "quality pass", "write findings");
	assert.equal(launched.status, 0, launched.stderr);
	const id = onlyJobId(launched.stdout);
	await waitForState(scratch.root, id, "done");
	const job = join(scratch.root, ".limen/jobs", id);
...
test("spawn --role loads that overlay preamble and persists the name", async (context) => {
	...
	const launched = limen(scratch, "spawn", "--detached", "--role", "researcher", "--label", "F069 research", "look around");
	assert.equal(launched.status, 0, launched.stderr);
	const id = onlyJobId(launched.stdout);
	await waitForState(scratch.root, id, "done");
	const job = join(scratch.root, ".limen/jobs", id);
...
test("spawn --role without a preamble or with --review plants no job", async (context) => {
	...
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const missing = limen(scratch, "spawn", "--detached", "--role", "ghost", "look around");
	assert.equal(missing.status, 1);
	assert.match(missing.stderr, /no preamble for role ghost/);
	const combined = limen(scratch, "spawn", "--detached", "--role", "researcher", "--review", "--branch", "limen/ghost", "inspect candidate");
...
`);
	context.after(scratch.cleanup);
	assert.equal(limen(scratch, "init").status, 0);
	const launched = limenWithEnv(scratch, { LIMEN_PREPARE: "touch prepared" }, "spawn", "--detached", "--label", "prep", "do work");
	assert.equal(launched.status, 0, launched.stderr);
	const id = onlyJobId(launched.stdout);
	await waitForState(scratch.root, id, "done");
	const worktree = (await readFile(join(scratch.root, ".limen/jobs", id, "worktree"), "utf8")).trim();
...
test("F074: a claude job keeps the ordinary record and files the closing result", async (context) => {
	...
	assert.match(limen(scratch, "continue", "--detached", id, "say more").stderr, /ran on claude/);
});
...
test("F074: a claude job refuses a hosted tab, and an unknown engine is named", async (context) => {
	...
	await writeFakeClaude(scratch.fakeBin, defaultFakeClaude);
	limen(scratch, "init");
	assert.match(limen(scratch, "spawn", "--engine", "claude", "look").stderr, /claude is not hosted in Herdr; pass --detached explicitly/);
	assert.match(limen(scratch, "spawn", "--detached", "--engine", "gpt", "look").stderr, /--engine must be pi or claude/);
