...
4. Menu assignment: OpenAI Luna→Terra→Sol→Astra, **Grok (first-class)**, opcjonalnie DeepSeek off-sub. Issue-fix: Luna, Terra albo Grok (DeepSeek tylko gdy assignment jawnie wybiera off-sub). Trudny brainstorm (design/aim/tech-review): Sol albo Astra; **plan-write: preferuj Terra lub Sol** (Astra tylko gdy trudność naprawdę wymaga — nie Astra-only). Lekki design/docs: Terra lub Grok OK. Każdy slot brainstorm = nowe assignment (aim, plan i tech-review nie dziedziczą Astry). Astra nie jest domyślnym modelem dla każdego zadania.
