...
Worker nie aktualizuje `notes.md`, GH, Plane ani boardu, nie spawnuje, nie merguje i nie deployuje. `done` dotyczy joba, nie akceptacji artefaktu ani zakończenia issue. Dla legacy zachowaj `to-grok.md` lub równoważnik; dla sealed managed result Router/Grok zapisuje osobny receiver receipt, a koordynator osobny verdict według [model-provenance.md](model-provenance.md). Nie dopisuj embedded ACK do zapieczętowanego wyniku. HTTP 2xx webhooka nie wystarcza. Koordynator musi najpierw odczytać i zachować dowody.
