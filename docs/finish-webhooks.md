...
2. Use Node 24+ and the reviewed checkout on this VPS. Run these commands in the
   operator's shell, retaining the variables through step 5. The checkout must
   be clean; record its revision before starting the hosted job.
   ...
   test -z "$(git -C "$LIMEN_ROOT" status --porcelain)" || { echo 'Dirty source; stop'; exit 1; }
