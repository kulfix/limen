...
Each owned terminal SDK execution writes `execution.json` before its terminal state becomes visible. The receipt always records the Limen job ID, backend, attempt, execution owner, start and finish times, limits, and terminal state. Validated observed model, auth route, working directory, permission mode, and SDK session ID appear only after the SDK establishes them; a failure before init never presents requested metadata as observed provenance. A wrapper that cannot establish ownership writes no execution receipt. `limen jobs <id>` shows the receipt and the existing finish-delivery evidence.
